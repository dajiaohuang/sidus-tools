...
import { equalStageMassRatio, normalizeEqualStageCount, TOOL_UNIT_SETS, toSi } from '@/lib/physics'
...
export function EqualStageTool() {
  ...
  const nStages = normalizeEqualStageCount(p.n)
  const res = useMemo(
    () => nStages === null ? null : equalStageMassRatio(dv, nStages, p.isp),
    [dv, nStages, p.isp],
  )
  return (
    <ToolShell
      parameters={<ParamsGrid>
        ...
        <UiField label={t('fields.number_of_equal_stages')} type="number" value={p.n} min={1} step={1} onChange={(e) => setP({ n: Number(e.target.value) })} />
      ...
      results={res && nStages !== null ? <div className="sidus-results">
        ...
        <ResultCard label={t('fields.overall_mass_ratio_ideal_equal')} value={(res.massRatio ** nStages).toFixed(3)} />
      </div> : <p className="font-mono text-sm text-muted">{t('fields.invalid_stage_budget')}</p>}
      code={nStages !== null ? <CodeExport formulaId="equal-stage" values={{ dv, n: nStages, isp: p.isp }} /> : null}
    />
  )
}
