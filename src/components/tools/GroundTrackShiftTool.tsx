...
export function GroundTrackShiftTool() {
  ...
  return (
    <ToolShell
      parameters={<ParamsGrid>
        ...
        <UiUnitField label={t('fields.altitude')} category="length" unitIds={TOOL_UNIT_SETS.altitude} unitId={p.hu} value={p.h} min={0} onValueChange={(h) => setP({ h })} onUnitChange={(hu, h) => setP({ hu, h })} hint={t('fields.hint_ground_track_shift')} />
        <FieldNote>
          {t('fields.note_ground_track_shift')}{' '}
          <Link to="/tools/ground-track" className="text-signal underline-offset-2 hover:underline">
            {t('fields.ground_track_map_link')}
          </Link>
        </FieldNote>
      </ParamsGrid>}
      ...
      code={<CodeExport formulaId="ground-track-shift" values={{ h, mu: body.mu, R: body.radius, body: p.body }} />}
    />
  )
}
