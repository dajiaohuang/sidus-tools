 ...
 */
const A =
  'Circular LEO; Ω̇_J2 equals mean solar motion over a tropical year; Earth J2, spherical. SI; h [m].'
...
omega_sun = 2 * math.pi / (365.24219 * 86400)
a = R + h
n = math.sqrt(mu / a**3)
...
const omegaSun = 2 * Math.PI / (365.24219 * 86400)
const a = R + h
...
const omegaSun: number = 2 * Math.PI / (365.24219 * 86400)
const a: number = R + h
...
const double omega_sun = 2.0 * M_PI / (365.24219 * 86400.0);
const double a = R + h;
...
let omega_sun = 2.0 * std::f64::consts::PI / (365.24219 * 86400.0);
let a = r_eq + h;
let n = (mu / (a * a * a)).sqrt();
...
const omega_sun = 2.0 * std.math.pi / (365.24219 * 86400.0);
const a = R + h;
...
mu = 3.986004418d14
R = 6378137.0d0
omega_sun = 2.0d0 * 3.141592653589793d0 / (365.24219d0 * 86400.0d0)
a = R + h
n = sqrt(mu / a**3)
...
mu = 3.986004418e14;
R = 6378137;
omega_sun = 2 * pi / (365.24219 * 86400);
a = R + h;
n = sqrt(mu / a^3);
cos_i = -(2/3) * (a / R)^2 * omega_sun / (n * J2);
