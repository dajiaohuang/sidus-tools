...
const ASSUMPTIONS =
  'Two-body problem; spherical central body; coplanar circular orbits; impulsive burns; dv1 and dv2 are nonnegative burn magnitudes; r1, r2 = circular-orbit radii (R + altitude), not the body radius; SI units (m, s).'
...
const double r1 = R + h1;
const double r2 = R + h2;
const double dv1 = std::fabs(std::sqrt(mu / r1) * (std::sqrt(2.0 * r2 / (r1 + r2)) - 1.0));
const double dv2 = std::fabs(std::sqrt(mu / r2) * (1.0 - std::sqrt(2.0 * r1 / (r1 + r2))));
const double a = 0.5 * (r1 + r2);
...
\\]
\\[
\\Delta v_1 = \\left|\\sqrt{\\frac{\\mu}{r_1}}\\!\\left(\\sqrt{\\frac{2r_2}{r_1+r_2}}-1\\right)\\right|
\\]
\\[
\\Delta v_2 = \\left|\\sqrt{\\frac{\\mu}{r_2}}\\!\\left(1-\\sqrt{\\frac{2r_1}{r_1+r_2}}\\right)\\right|
\\]`,
  },
}
